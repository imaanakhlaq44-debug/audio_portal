...
function parseSeries(file) {
  ...
  // `const Series adamEnglish = Series(` -> adamEnglish, which is how
  // StoryData.allSeries names it.
  const variable = src.match(/const Series (\w+) = Series\(/)?.[1];
...
// The app's own running order, from StoryData.allSeries: English before
// Urdu, and the Prophets in the order they came rather than the order their
// files happen to sort in. Reading it from there rather than repeating the
// rule here is what keeps a new series from landing in a different place on
// the website than in the app - Lut before Ibrahim, say, which would give
// away a story that opens on the end of the one before it.
const order = (() => {
  const src = readFileSync(
    join(here, '..', '..', 'lib', 'models', 'story_data.dart'),
    'utf8',
  );
  const list = src.slice(
    src.indexOf('allSeries = ['),
    src.indexOf('];', src.indexOf('allSeries = [')),
  );
  return [...list.matchAll(/^\s{4}([a-zA-Z]+),$/gm)].map((m) => m[1]);
})();
...
const all = readdirSync(seriesDir)
  ...
  .sort((a, b) => {
    const rank = (s) => {
      const i = order.indexOf(s.variable);
      return i < 0 ? order.length : i;
    };
    return rank(a) - rank(b);
  });

const unplaced = all.filter((s) => !order.includes(s.variable));
if (unplaced.length) {
  throw new Error(
    `not in StoryData.allSeries, so the website has nowhere to put them: ` +
      unplaced.map((s) => s.id).join(', '),
  );
}
...
// The Dart variable name is only how the order above is read; the website
// has no use for it.
const shipped = all.map(({ variable, ...series }) => series);

mkdirSync(dirname(outFile), { recursive: true });
writeFileSync(outFile, banner + JSON.stringify(shipped, null, 2) + ';\n');
